...
unsigned long SENSOR_UPDATE_TIME_MS = 200;
unsigned long COFFEE_BREWING_TIME_MS = 5000;
...
class Device {
	private:
		// Define Pins here
		int waterSensorPin = D2;
		int cupSensorPin = D3;
		int makeCoffeePin = 5;
		bool lastWaterSensorReading = false;
		bool lastCupSensorReading = false;
		bool makingCoffee = false;
	public:
		Device() {
			pinMode(waterSensorPin, INPUT);
			pinMode(cupSensorPin, INPUT);
			pinMode(makeCoffeePin, OUTPUT);
		}
		...
		void setState(String jsonString) {
			...
			bool activateCoffeeMachine = doc["makeCoffee"];
			Serial.print("Make coffee command received: ");
			Serial.println(activateCoffeeMachine?"true":"false");
			if (activateCoffeeMachine && 
					lastWaterSensorReading && 
					lastCupSensorReading) {
				makingCoffee = true;
			}
		...
		void sendSensorData(WiFiClient listenerServer) {
			...
			doc["makeCoffee"] = makingCoffee;
			doc["cupPresent"] = lastCupSensorReading;
			doc["enoughWater"] = lastWaterSensorReading;

			Serial.println("Sending new sensor readings to RPi");
		...
		bool hasSensorChanged() {
			...
			int waterSensorSignal = digitalRead(waterSensorPin);
			bool newValue = (waterSensorSignal == HIGH);
			if (newValue != lastWaterSensorReading) {
				lastWaterSensorReading = newValue;
				result = true;
			}
			
			int cupSensorSignal = digitalRead(cupSensorPin);
			newValue = (cupSensorSignal == HIGH);
			if (newValue != lastCupSensorReading) {
				lastCupSensorReading = newValue;
				result = true;
			}
		...
		bool shouldMakeCoffee() {
			return makingCoffee;
		}

		void startMakingCoffee() {
			digitalWrite(makeCoffeePin, HIGH);
			makingCoffee = true;
		}

		void stopMakingCoffee() {
			digitalWrite(makeCoffeePin, LOW);
			makingCoffee = false;
		}
};
...
void loop()
{
	...
	if (thisDevice.shouldMakeCoffee()) {
		thisDevice.startMakingCoffee();
		delay(COFFEE_BREWING_TIME_MS);
	}
...
void updateData() {
	if (thisDevice.hasSensorChanged()) {
		WiFiClient client;
		if (!client.connect(raspberryIp, TCP_PORT)) {
			Serial.println("Connection failed, restarting UDP exchange");
			setupIpExchange();
			return;
		}
