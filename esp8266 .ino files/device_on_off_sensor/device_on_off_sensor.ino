...
class Device {
	...
	public:
		...
		void setState(String jsonString) {
			// Device does not have attributes to set
		}
		...
		void sendSensorData(WiFiClient listenerServer) {
			...
			doc["isActive"] = lastSensorReading;
			
			serializeJson(doc, listenerServer);
		}

		bool hasSensorChanged() {
			int sensorSignal = digitalRead(sensorPin);
			bool newValue = (sensorSignal == HIGH);
			if (newValue != lastSensorReading) {
				lastSensorReading = newValue;
				return true;
			}
			return false;
		}
