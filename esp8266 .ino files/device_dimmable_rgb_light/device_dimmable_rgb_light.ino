...
class Device {
	private:
		...
		void applyNewLights() {
			int brightnessHexa = (int)((brightness/100.0)*255);
			
			if (isOn) {
				analogWrite(whitePin, isWhite ? brightnessHexa : 0);
				analogWrite(redPin, isWhite ? 0 : (int)(red)*(brightness/100.0));
				analogWrite(grnPin, isWhite ? 0 : (int)(grn)*(brightness/100.0));
				analogWrite(bluPin, isWhite ? 0 : (int)(blu)*(brightness/100.0));
			} else {
	...
	public:
		...
		void setState(String jsonString) {
			...
			const size_t capacity = JSON_OBJECT_SIZE(10) + 160;
